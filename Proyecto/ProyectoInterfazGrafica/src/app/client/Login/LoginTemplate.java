...
public class LoginTemplate extends JFrame{
    ...
    private void crearJPasswordFields(){
        ...
        tClaveUsuario = sObjGraficos.construirJPasswordField("Clave Usuario", 
                ...
                sRecursos.getFontPanel(),  sRecursos.getBordeVerde(), "c"
        );
    ...
    private void crearJLabels(){
        ...
        lEslogan = sObjGraficos.construirJLabel(
                ...
                null, Color.WHITE, null, sRecursos.getFontTPrincipal()
        );
        ...
        lTituloLogin = sObjGraficos.construirJLabel(
                ...
                Color.WHITE, null, sRecursos.getFontTitulo()
        );
        ...
        lCondiciones = sObjGraficos.construirJLabel(
                "Si haces clic en iniciar sesión con Facebook y no eres usuario "
                + "de Spotify, pasarás a estar", (pInferior.getWidth()-400)/2, 
                475, 400, 15, null, Color.WHITE, null, sRecursos.getFontCondiciones()
        );
        pInferior.add(lCondiciones);
        
        lCondiciones2 = sObjGraficos.construirJLabel(
                "registrado y aceptas los Términos y Condiciones y la Política "
                + "de Privacidad de Spotify.", (pInferior.getWidth()-415)/2, 
                490, 415, 15, null, Color.WHITE, null, sRecursos.getFontCondiciones()
        );
        pInferior.add(lCondiciones2);
        
        
        iDimAux = new ImageIcon(iLogo.getImage().getScaledInstance(250, 95, Image.SCALE_AREA_AVERAGING));
        lLogo = sObjGraficos.construirJLabel(null, (pSuperior.getWidth()-250)/2,
                22, 250, 95, iDimAux, null, null, null
        );
        pSuperior.add(lLogo);
        
        iDimAux = new ImageIcon(iFacebook.getImage().getScaledInstance(30, 30, Image.SCALE_AREA_AVERAGING));
        lFacebook = sObjGraficos.construirJLabel(null, 12, 470, 30, 30, iDimAux, 
                null, null, null
        );
        pInferior.add(lFacebook);

        iDimAux = new ImageIcon(iTwitter.getImage().getScaledInstance(30, 30, Image.SCALE_AREA_AVERAGING));
        lTwitter = sObjGraficos.construirJLabel(null, 52, 470, 30, 30, iDimAux, 
                null, null, null
        );
        pInferior.add(lTwitter);

        iDimAux = new ImageIcon(iYoutube.getImage().getScaledInstance(30, 30, Image.SCALE_AREA_AVERAGING));
        lYoutube = sObjGraficos.construirJLabel(null, 92, 470, 30, 30, iDimAux, 
                null, null, null
        );
        pInferior.add(lYoutube);
        
        iDimAux = new ImageIcon(iUsuario.getImage().getScaledInstance(30, 30, Image.SCALE_AREA_AVERAGING));
        lUsuario = sObjGraficos.construirJLabel(null, 355, 130, 30, 30, iDimAux, 
                null, null, null
        );
        pInferior.add(lUsuario);

        iDimAux = new ImageIcon(iClave.getImage().getScaledInstance(30, 30, Image.SCALE_AREA_AVERAGING));
        lClave = sObjGraficos.construirJLabel(null, 355, 185, 30, 30, iDimAux, 
                null, null, null
        );
